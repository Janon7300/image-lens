...
async function credentials() {
  ...
  if (!await chrome.permissions.request({origins: [ep.origin]})) throw new Error('ยังไม่ได้อนุญาตบริการ');
  return {ep, key};
}
...
$('form').onsubmit = async event => {
  ...
  try {
    ...
    const allowed = await chrome.permissions.request({origins: [ep.origin]});
    if (!allowed) throw new Error('ยังไม่ได้อนุญาตให้เชื่อมต่อบริการ');
    await chrome.storage.local.set({settings: {endpoint: ep.url, model, language: $('language').value, censorMode: $('censorMode').value, consent: $('consent').checked}});
