(() => {
  ...
  `;
  root.append(theme);
  if (globalThis.FontFace && browser.runtime.getURL) {
    const font = new FontFace('Kodchasan', `url("${browser.runtime.getURL('fonts/Kodchasan-Regular.ttf')}")`);
  ...
  async function translate(automatic = false) {
    ...
    try {
      ...
      if (automatic) count++;
      const pending = browser.runtime.sendMessage({type: 'translate'});
  ...
  function visibility() {
    if (document.hidden) { revision++; clearTimeout(timer); browser.runtime.sendMessage({type: 'cancel'}).catch(() => {}); }
  }
  function captureDone(message) {
    if (message.type === 'captureDone' && !closed) $('bar').style.visibility = 'visible';
  }
  browser.runtime.onMessage.addListener(captureDone);
  ...
  $('close').onclick = () => {
    ...
    browser.runtime.onMessage.removeListener(captureDone);
